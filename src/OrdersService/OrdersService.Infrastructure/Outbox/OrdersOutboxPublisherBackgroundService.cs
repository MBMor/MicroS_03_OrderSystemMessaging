    ...
    ILogger<OrdersOutboxPublisherBackgroundService> logger) : BackgroundService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogInformation(
                "Publishing Orders outbox message {OutboxMessageId}. EventId: {EventId}, EventType: {EventType}, RoutingKey: {RoutingKey}, RetryCount: {RetryCount}, ExchangeName: {ExchangeName}",
                outboxMessage.Id,
            ...
            _logger.LogInformation(
                "Orders outbox message {OutboxMessageId} published. EventId: {EventId}, EventType: {EventType}, RoutingKey: {RoutingKey}, RetryCount: {RetryCount}, Status: {Status}",
                outboxMessage.Id,
                outboxMessage.EventId,
                outboxMessage.EventType,
                outboxMessage.RoutingKey,
        ...
        catch (Exception exception) when (exception is not OperationCanceledException)
        {
            ...
            _logger.Log(
                logLevel,
                exception,
                "Publishing Orders outbox message {OutboxMessageId} failed. EventId: {EventId}, EventType: {EventType}, RoutingKey: {RoutingKey}, RetryCount: {RetryCount}, MaxRetryCount: {MaxRetryCount}, Status: {Status}",
                outboxMessage.Id,
                outboxMessage.EventId,
                outboxMessage.EventType,
                outboxMessage.RoutingKey,
