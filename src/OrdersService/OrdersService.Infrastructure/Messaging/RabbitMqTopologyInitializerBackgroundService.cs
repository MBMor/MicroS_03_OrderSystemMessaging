    ...
    ILogger<RabbitMqTopologyInitializerBackgroundService> logger) : BackgroundService
{
    ...
    {
        _logger.LogInformation("Orders RabbitMQ topology initializer is starting.");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                _logger.LogInformation("Orders RabbitMQ topology initializer completed successfully.");
            ...
            catch (Exception exception)
            {
                _logger.LogWarning(
                    exception,
                    "Orders RabbitMQ topology initialization failed. Retrying in {RetryDelaySeconds} second(s).",
