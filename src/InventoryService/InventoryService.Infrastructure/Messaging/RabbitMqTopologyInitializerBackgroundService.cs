    ...
    ILogger<RabbitMqTopologyInitializerBackgroundService> logger) : BackgroundService
{
    ...
    {
        _logger.LogInformation("Inventory RabbitMQ topology initializer is starting.");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                _logger.LogInformation("Inventory RabbitMQ topology initializer completed successfully.");
            ...
            catch (Exception exception)
            {
                _logger.LogWarning(
                    exception,
                    "Inventory RabbitMQ topology initialization failed. Retrying in {RetryDelaySeconds} second(s).",
